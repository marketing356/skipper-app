...
export async function GET(req: NextRequest) {
  ...
  const res = await fetch(`${E}/api/v1/boater/contact-notes${qs ? '?' + qs : ''}`, { headers: H() })
  return NextResponse.json(await res.json(), { status: res.status })
}
export async function POST(req: NextRequest) {
  const res = await fetch(`${E}/api/v1/boater/contact-notes`, { method: 'POST', headers: H(), body: JSON.stringify(await req.json()) })
  return NextResponse.json(await res.json(), { status: res.status })
}
