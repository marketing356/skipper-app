/**
 * PATCH /api/push-token — Save boater Expo push token via Railway (Rule 2 compliant)
 */
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const res = await fetch(`${E}/api/v1/boater/push-token`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json', 'x-skipper-api-key': K, 'x-boater-auth': auth },
