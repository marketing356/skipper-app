'use client'
/**
 * EmergencyContactList — Mobile version. Inline styles matching ContactForm dark theme.
...
export default function EmergencyContactList({ contactId, marinaId }: EmergencyContactListProps) {
  const [rows, setRows] = useState<ECRow[]>([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    if (!contactId) return
    fetch(`/api/emergency-contacts?contact_id=${contactId}`)
      .then((r) => r.json())
      .then((data) => {
  ...
  function addRow() {
    setRows((prev) => [
      ...prev,
      { name: '', phone: '', relationship: '', notes: '', expanded: true, status: 'new' },
    ])
  }

  function updateRow(idx: number, patch: Partial<ECRow>) {
  ...
  async function saveRow(idx: number) {
    ...
    try {
      if (row.id) {
        const res = await fetch(`/api/emergency-contacts/${row.id}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ name: row.name, phone: row.phone, relationship: row.relationship, notes: row.notes }),
        })
        ...
        const res = await fetch('/api/emergency-contacts', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ contact_id: contactId, marina_id: marinaId, name: row.name, phone: row.phone, relationship: row.relationship, notes: row.notes }),
        })
        if (!res.ok) throw new Error('Save failed')
  ...
  async function deleteRow(idx: number) {
    ...
    try {
      await fetch(`/api/emergency-contacts/${row.id}`, { method: 'DELETE' })
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      {rows.map((row, idx) => {
        const summary = row.name
          ? `${row.name}${row.relationship ? ` — ${row.relationship}` : ''}`
              ...
              <div style={{ padding: '12px 14px', background: 'rgba(255,255,255,0.02)', display: 'flex', flexDirection: 'column', gap: 8 }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                  <input type="text" placeholder="Full name" value={row.name}
                    onChange={(e) => updateRow(idx, { name: e.target.value })} style={inputStyle} />
                  <input type="tel" placeholder="Phone" value={row.phone}
                    onChange={(e) => updateRow(idx, { phone: e.target.value })} style={inputStyle} />
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                  <input type="text" placeholder="Relationship (e.g. Spouse)" value={row.relationship}
                    onChange={(e) => updateRow(idx, { relationship: e.target.value })} style={inputStyle} />
      ...
      <button type="button" onClick={addRow}
        ...
        <span style={{ fontSize: 18, lineHeight: 1 }}>＋</span> Add emergency contact
      </button>
    </div>
  )
}
