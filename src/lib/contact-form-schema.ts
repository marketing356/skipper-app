...
// ─── Runtime fetch from OPS ────────────────────────────────────────────────
// Fetches live schema from OPS. Falls back to static array below if OPS is unreachable.
export async function fetchContactFormSchema(): Promise<ContactSection[]> {
  try {
    ...
    if (!res.ok) throw new Error(`OPS schema ${res.status}`)
